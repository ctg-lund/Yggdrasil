...
mkdir -p ${runfolder_name}_interop_qc/"${runfolder_name}"

# Generating interop_summary (Python): "
interop_summary ${runfolder_path} --csv=1 > \
    ${runfolder_name}_interop_qc/"${runfolder_name}"/interop_summary

# Generating interop_index-summary (Python): "
interop_index-summary ${runfolder_path} --csv=1 > \
    ${runfolder_name}_interop_qc/"${runfolder_name}"/interop_index-summary
...
mv ${runfolder_name}*_mqc.png ${runfolder_name}_interop_qc/"${runfolder_name}"/

# MultiQC
multiqc -f ${runfolder_name}_interop_qc/"${runfolder_name}"/ -n ${runfolder_name}_interop_qc/multiqc_${runfolder_name} 

mv ${runfolder_name}_interop_qc/multiqc_${runfolder_name}.html /projects/fs1/shared/ctg-qc/qcapp2/sequencing-runs/${runfolder_name}/qc/interop/multiqc_report.html

rm -r ${runfolder_name}_interop_qc
